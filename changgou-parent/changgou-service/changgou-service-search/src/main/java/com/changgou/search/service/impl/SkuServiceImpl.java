...
//        String category = searchMap.get("category");
        ...
        //6.获取结果 封装返回
        //6.1 获取当前页的记录
        List<SkuInfo> content = skuPage.getContent();//该content 内容是无高亮的数据
        //6.2 获取根据条件命中的总记录数
...
/*
        Map<String, Set<String>> result = stringTermsSpec.getBuckets().stream()
                .map(s -> JSON.parseObject(s.getKeyAsString(), Map.class))
                .map((Function<Map, Set>) Map::entrySet)
                .map((Function<Set, Stream>) Collection::stream)
        ...
        if(stringTermsSpec!=null){
            ...
            for (StringTerms.Bucket bucket : stringTermsSpec.getBuckets()) {
                ...
                Map<String, Set<String>> result = map.entrySet().stream().collect(Collectors.groupingBy(Map.Entry::getKey, Collectors.mapping(Map.Entry::getValue, Collectors.toSet())));
...
//                    Set<String> values = new HashSet<String>();
    ...
    private List<String> getStringsList(StringTerms stringTermsBrand) {
        ...
        if (stringTermsBrand != null) {
            return stringTermsBrand.getBuckets().stream().map(StringTerms.Bucket::getKeyAsString).collect(Collectors.toList());
        }
        return null;
    }
